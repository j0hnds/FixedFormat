    ...
    public Object createRecord(String record) {
        if (recordIdentifier == null) {
            throw new IllegalStateException(
                    "No record identifier configured for this factory");
        }

        String recordId = recordIdentifier.getRecordIdentifier(record);
        if (recordId == null) {
            throw new RuntimeException("No record identifier found in record: "
                    + record);
        }

        if (recordFormats == null) {
            throw new IllegalStateException(
                    "No Record Format Map configured for this factory");
        }

        Record recordFormat = (Record) recordFormats.get(recordId);
        if (recordFormat == null) {
            throw new IllegalStateException(
                    "No record format configured for this record ID: "
                            + recordId);
        }

        if (recordDataMap == null) {
            throw new IllegalStateException(
                    "No record data map configured for this factory");
        }

        Object recordData = recordDataMap.get(recordId);
        if (recordData == null) {
            throw new IllegalStateException(
                    "No record data object configured for this record ID:"
                            + recordId);
        }
